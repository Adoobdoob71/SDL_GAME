...
void Ball::Lock(int WIDTH, int HEIGHT) {
	...
	for (GameObject *player : ov->_objectV) {
		if (ypos >= player->ypos - 50 && ypos <= player->ypos + 50 && (xpos + 50 == player->xpos || xpos == player->xpos + player->destR.w))
			Xvelocity *= -1;
		if (xpos >= player->xpos - 50 && xpos <= player->xpos + 50 && (ypos + 50 == player->ypos || ypos == player->ypos + player->destR.h))
			Yvelocity *= -1;
	}
}
